	...
	for(let i = 0; i < neighbours.length; i++){ 
 	...
 	// consider node only if its not a wall and if its not being looked at already 
 		...
 		if(tentativeDist < neighbour.distance){  	
 			...
 			neighbour.total = heuristic(neighbour, end); // total cost estimate. We do not consider neighbours distance for minimization of total cost in case of greedy BFS 
 		} 
	} 
  	let minNode = 0; 
